...
import React, { useRef, useEffect, useMemo, type JSX } from 'react'
...
function PointCloudMesh({ mesh_path, ...props }) {
  const scroll = useScroll()
  const { nodes, materials } = useGLTF(mesh_path) as GLTFResult
  ...
  const mesh = useMemo(() => new THREE.Mesh(nodes.model.geometry), [nodes.model.geometry]);
...
function PhysicalMesh({mesh_path, ...props }) { 
  const scroll = useScroll()
  const ref = useRef()
  const { nodes, materials } = useGLTF(mesh_path) as GLTFResult
...
function BBoxMesh({mesh_path, ...props }) { 
  const { nodes, materials } = useGLTF(mesh_path) as GLTFResult
...
export function BaseSKU({mesh_path, ...props}) {
  return (
